    ...
    def Move(self):
        if self.snake_tail > 0:
            for i in range(self.snake_tail, 0, -1):
                self.snake[i][0] = self.snake[i - 1][0]
                self.snake[i][1] = self.snake[i - 1][1]
            if ((self.snake[0][0] + direction[0]) % WIDTH != self.snake[1][0]) and ((self.snake[0][1] + direction[1]) % HEIGHT != self.snake[1][0]):
                self.snake[0][0] = (self.snake[0][0] + direction[0]) % WIDTH
                self.snake[0][1] = (self.snake[0][1] + direction[1]) % HEIGHT
            else:
                self.snake[0][0] = ((self.snake[0][0] - self.snake[1][0]) + self.snake[0][0]) % WIDTH
                self.snake[0][1] = ((self.snake[0][1] - self.snake[1][1]) + self.snake[0][1]) % HEIGHT
        else:
            self.snake[0][0] = (self.snake[0][0] + direction[0]) % WIDTH
            self.snake[0][1] = (self.snake[0][1] + direction[1]) % HEIGHT
